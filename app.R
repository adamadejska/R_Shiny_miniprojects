...
pbmc <- 1  # Placeholder for the global Seurat object
...
ui <- navbarPage("Seurat tutorial",
                 tabPanel("Data Input", 
                          ...
                          helpText("Specify the location of the data directory containing the 
                                    matrix.mtx, genes.tsv (or features.tsv), and barcodes.tsv 
                                    files provided by 10X."),
                          shinyDirButton("dir", "Input directory", "Upload"),
                          verbatimTextOutput("dir", placeholder = TRUE),
                          actionButton("dir_update", "Update"),
                          helpText("Please wait until the notification informs you the data was successfully uploaded.")
                 ),
                 tabPanel("QC metrics", 
                          helpText("This is an app that follows the steps of the Seurat tutorial.
                     Use each tab to explore the different steps of the tutorial."),
...
server <-function(input, output) {
  ...
  # Make sure we are able to see the directories and choose one.
  shinyDirChoose(
    input,
    'dir',
    roots = c(home = '~'),
    filetypes = c('', 'txt', 'bigWig', "tsv", "csv", "bw", "mtx")
  )
  ...
  observe({
    if(input$dir_update > 0){
      print(output_dir())
      pbmc.data <- tryCatch({
        Read10X(data.dir =  output_dir())
      }, error=function(e) {
        showNotification("Incorrect input directory", type="error")
      }, warning=function(w) {
        showNotification("Something went wrong", type="error")
      })
      showNotification("Uploading the data. Please wait.")
      ...
      showNotification("Done uploading the data", duration=NULL, type="message")
    }
  })
  ...
  output$topN <- renderTable({
    ...
    pbmc <<- FindVariableFeatures(pbmc, selection.method = "vst", nfeatures = 2000)
    as_tibble(HVFInfo(pbmc),rownames = "Gene") -> hvftable
    hvftable %>%
      mutate(hypervariable=Gene %in% VariableFeatures(pbmc)
      ) -> hvftable
    
    topN <- head(VariableFeatures(pbmc), input$topNfeautres)
    topNtable <- filter(hvftable, Gene%in%topN)
    topNtable <- topNtable[order(-topNtable$"variance.standardized"),]
    topNtable
